...
def function1():
    ...
    x = np.arange(1,13,1).reshape(6,2)
...
def function2():
    ...
    x = np.arange(10,37,1,dtype=np.float64).reshape(3,3,3)
...
def function8():
    ...
    x = ((np.ones(10, dtype=np.uint32)).reshape(2,5))*6
...
def function12():
    ...
    ans2 = np.concatenate(ans1)
    ans3 = np.hstack((arr, arr))
    ans = np.concatenate((ans2, ans3))
...
def function18():
    ...
    x = np.count_nonzero(np.in1d(piaic, students) == True)
    return x
